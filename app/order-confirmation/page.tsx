...
export default async function OrderConfirmation({
  ...
  const totals = items.reduce(
    (acc, item) => {
      ...
      acc.pounds += poundsFromBags(item.qty_bags, product?.bag_size_lb ?? 0.5);
      return acc;
    },
    { bags: 0, pounds: 0 }
  );
  ...
  const pickupLocation = Array.isArray(pickup?.pickup_location) ? pickup?.pickup_location[0] : pickup?.pickup_location;
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex justify-between">
                    <span>
                      {product?.name ?? 'Product'} · {item.qty_bags} bag{item.qty_bags === 1 ? '' : 's'}
                    </span>
                    <span className="text-slate-100">
                      {poundsFromBags(item.qty_bags, product?.bag_size_lb ?? 0.5).toFixed(1)} lbs
                    </span>
                  </div>
                );
              })}
          ...
          <div className="card space-y-3">
            ...
            <p className="text-slate-200">{pickupLocation?.name ?? 'Pickup location'}</p>
            <p className="text-sm text-slate-300">{formatWindow(pickup?.start_time, pickup?.end_time)}</p>
            {pickup?.instructions && <p className="text-xs text-slate-400">{pickup.instructions}</p>}
