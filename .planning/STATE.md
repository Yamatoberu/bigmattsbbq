...
| 260918-wec | Trigger Resend welcome-email automation on mailing-list signup (issue #18) — optional `RESEND_WELCOME_EVENT` env var added to `ResendEnv`; `POST /api/mailing-list` now pre-checks `contacts.get({ email, audienceId })` before `contacts.create` and, only on a successful create, fires `resend.events.send({ event, email, payload: { FIRST_NAME } })` inside `after()` for new subscribers and previously-unsubscribed re-signups (active duplicates and lookup failures skip — fail-closed, since Resend creates an automation run on every event with no server-side dedup); trigger failures are swallowed with `console.warn` and never affect the response. 8 new tests (17 in file), 322/322 total pass, tsc clean. Resend-side automation (custom event + Trigger → Send Email mapping `FIRST_NAME` ← `event.FIRST_NAME`) and setting `RESEND_WELCOME_EVENT` in Vercel are the user's follow-up per the design on issue #18 | 2026-09-19 | bdb894e | [260918-wec-trigger-resend-welcome-email-automation-](./quick/260918-wec-trigger-resend-welcome-email-automation-/) |
